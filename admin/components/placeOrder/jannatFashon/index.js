...
const validationSchema = Yup.object().shape({
  ...
  note: Yup.string()
    .max(400)
    .default(
      "দয়া করে সাবধানে এবং দ্রুত ডেলিভারি করবেন। ধন্যবাদ"
    ),
...
const AddJFOrder = ({ onClick }) => {
  ...
  const sendConfirmationMsg = async (values, orderID, tracking_code = "") => {
    ...
    const message = `Dear ${customer_name}, Your order has been successfully placed at Jannat Fashon. Invoice No: ${orderID}. Please keep BDT: ${
      values?.salePrice
    }tk ready while receiving the parcel. Hotline: +88${company_contact}. Thanks for being with us.`;
